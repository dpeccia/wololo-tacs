...
class GameTests {
    ...
    inner class GameCreation {
        ...
        fun `creating a game distributes towns with the available players evenly`() {
            val game = Game(players = players, province = Province(0, "a_province", ArrayList(towns)))
        ...
        fun `Attempting to create a game with more players than towns throws IlegalGameException`() {
            assertThrows<CustomException.BadRequest.IllegalGameException> { Game(players = players, province = Province(0, "a_province", ArrayList(listOf(town1)))) }
        }

        @Test
        fun `Attempting to create a game without players throws IlegalGameException`() {
            assertThrows<CustomException.BadRequest.IllegalGameException> { Game(players = listOf(), province = Province(0, "a_province", ArrayList(towns))) }
        }

        @Test
        fun `Can successfully create a game with none empty list of players`() {
            assertDoesNotThrow { Game(players = players, province = Province(0, "a_province", ArrayList(towns))) }
        }
    ...
    inner class MoveGauchos {
        ...
        private val game = Game(players = listOf(user1), province = Province(0, "a_province", ArrayList(towns)))
        ...
        fun `trying to move gauchos when it is not your turn throws NotYourTurnException`() {
            val game2 = Game(players = listOf(user1, user2), province = Province(0, "a_province", ArrayList(towns)))
            game2.turn = user1
    ...
    inner class Turn {
        ...
        fun `successfully finish turn unlocks all towns from User`() {
            val game2 = Game(players = listOf(user1, user2), province = Province(0, "a_province", arrayListOf(town1, town2, town3, town4, town5)))
            game2.turn = user1
        ...
        fun `if the first player finishes his turn and he didnt win, turn is changed to next player`() {
            ...
            val game2 = Game(players = listOf(user1, user2, user3), province = Province(0, "a_province", arrayListOf(town1, town2, town3, town4, town5)))
            game2.turn = user1
            game2.finishTurn(user1)
